...
out=${LY_ROUTE_VPP_DNS_INTERCEPT_BUILD_DIR:-$repo_root/build/vpp-dns-intercept}
sysroot=${LY_ROUTE_VPP_DNS_INTERCEPT_SYSROOT:-$out/sysroot}
...
cmake -S "$repo_root/runtime/vpp-dns-intercept" -B "$out/cmake" \
...
plugin=$(find "$out/cmake" -type f -name ly_route_dns_intercept_plugin.so -print -quit)
[ -n "$plugin" ] || { echo "DNS intercept plugin was not produced" >&2; exit 1; }
printf '%s\n' "$plugin"
