...
out=${LY_ROUTE_VPP_NAT_CONTEXT_BUILD_DIR:-$repo_root/build/vpp-nat-context}
sysroot=${LY_ROUTE_VPP_NAT_CONTEXT_SYSROOT:-$out/sysroot}
...
cmake -S "$repo_root/runtime/vpp-nat-context" -B "$out/cmake" \
...
plugin=$(find "$out/cmake" -type f -name ly_route_nat_context_plugin.so -print -quit)
[ -n "$plugin" ] || { echo "NAT context plugin was not produced" >&2; exit 1; }
printf '%s\n' "$plugin"
