...
patch_dir="$repo_root/packaging/vpp-patches"
...
git -C "$rootfs/src/vpp" init -q
if [ -d "$patch_dir" ]; then
  for patch in "$patch_dir"/*.patch; do
    [ -f "$patch" ] || continue
    if git -C "$rootfs/src/vpp" apply --check "$patch"; then
      git -C "$rootfs/src/vpp" apply "$patch"
    elif git -C "$rootfs/src/vpp" apply --reverse --check "$patch"; then
      printf 'VPP patch already present: %s\n' "$(basename "$patch")"
    else
      echo "VPP patch does not match the pinned source: $patch" >&2
      exit 1
    fi
  done
fi
grep -q 'vlib_pci_map_dma' "$rootfs/src/vpp/src/plugins/vmxnet3/vmxnet3.c" || {
  echo 'VMXNET3 DMA patch is missing from the pinned VPP source' >&2
  exit 1
}
