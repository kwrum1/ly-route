...
out=${LY_ROUTE_VPP_PRE_NAT_ROUTE_BUILD_DIR:-$repo_root/build/vpp-pre-nat-route}
sysroot=${LY_ROUTE_VPP_PRE_NAT_ROUTE_SYSROOT:-$out/sysroot}
...
cmake -S "$repo_root/runtime/vpp-pre-nat-route" -B "$out/cmake" \
...
plugin=$(find "$out/cmake" -type f -name ly_route_pre_nat_route_plugin.so -print -quit)
[ -n "$plugin" ] || { echo "pre-NAT route plugin was not produced" >&2; exit 1; }
printf '%s\n' "$plugin"
