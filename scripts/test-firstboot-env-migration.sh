...
"$repo_root/packaging/rootfs-overlay/usr/lib/ly-route/migrate-control-env.sh" "$control_env" eth0 192.168.88.1/24
"$repo_root/packaging/rootfs-overlay/usr/lib/ly-route/migrate-control-env.sh" "$control_env" eth0 192.168.88.1/24

grep -q '^LY_ROUTE_MANAGEMENT_INTERFACE=eth0$' "$control_env"
grep -q '^LY_ROUTE_LAN_CIDR=192.168.88.1/24$' "$control_env"
