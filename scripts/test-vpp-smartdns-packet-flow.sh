...
func main() {
  ...
  ip := frame[14:]; ip[0]=0x45; binary.BigEndian.PutUint16(ip[2:4], uint16(20+8+len(dns))); ip[8]=64; ip[9]=17; copy(ip[12:16], []byte{192,0,2,10}); copy(ip[16:20], []byte{192,0,2,1}); binary.BigEndian.PutUint16(ip[10:12], checksum(ip[:20]))
...
func makeSegment(iface *net.Interface, seq, ack uint32, flags byte, payload []byte) []byte {
  ...
  ip := frame[14:]; ip[0] = 0x45; binary.BigEndian.PutUint16(ip[2:4], uint16(40+len(payload))); ip[8] = 64; ip[9] = 6; copy(ip[12:16], []byte{192,0,2,10}); copy(ip[16:20], []byte{192,0,2,1}); binary.BigEndian.PutUint16(ip[10:12], checksum(ip[:20]))
...
func main() {
  ...
  for time.Now().Before(deadline) { n, _, e := syscall.Recvfrom(fd, buf, 0); if e != nil { time.Sleep(10*time.Millisecond); continue }; if n < 54 || binary.BigEndian.Uint16(buf[12:14]) != 0x0800 || buf[23] != 6 { continue }; ihl := int(buf[14]&15)*4; tcp := buf[14+ihl:n]; if len(tcp) < 20 || binary.BigEndian.Uint16(tcp[0:2]) != 53 || binary.BigEndian.Uint16(tcp[2:4]) != 40001 { continue }; flags := tcp[13]; if flags&0x12 == 0x12 { serverSeq = binary.BigEndian.Uint32(tcp[4:8]); if err := syscall.Sendto(fd, makeSegment(iface, clientSeq+1, serverSeq+1, 0x10, nil), 0, addr); err != nil { panic(err) }; if err := syscall.Sendto(fd, makeSegment(iface, clientSeq+1, serverSeq+1, 0x18, payload), 0, addr); err != nil { panic(err) }; break } }
  if serverSeq == 0 { panic("timed out waiting for TCP SYN/ACK") }
  for time.Now().Before(deadline.Add(5*time.Second)) { n, _, e := syscall.Recvfrom(fd, buf, 0); if e != nil { time.Sleep(10*time.Millisecond); continue }; if n < 54 || binary.BigEndian.Uint16(buf[12:14]) != 0x0800 || buf[23] != 6 { continue }; ihl := int(buf[14]&15)*4; tcp := buf[14+ihl:n]; if len(tcp) < 20 || binary.BigEndian.Uint16(tcp[0:2]) != 53 || binary.BigEndian.Uint16(tcp[2:4]) != 40001 { continue }; offset := int(tcp[12]>>4)*4; if len(tcp) < offset { continue }; data := tcp[offset:]; if bytes.Contains(data, []byte{203,0,113,53}) { fmt.Println("VPP DNS TCP packet response received"); return }; }
...
func main() {
  ...
  src := net.ParseIP("2001:db8:1::10").To16(); dst := net.ParseIP("2001:db8:1::1").To16(); qname := []byte{7,'u','p','d','a','t','e','s',7,'e','x','a','m','p','l','e',0}; dns := make([]byte, 12+len(qname)+4); binary.BigEndian.PutUint16(dns[0:2], 0xcafe); binary.BigEndian.PutUint16(dns[2:4], 0x0100); binary.BigEndian.PutUint16(dns[4:6], 1); copy(dns[12:], qname); binary.BigEndian.PutUint16(dns[12+len(qname):], 1); binary.BigEndian.PutUint16(dns[14+len(qname):], 1)
  ...
  deadline := time.Now().Add(5*time.Second); buf := make([]byte, 4096); for time.Now().Before(deadline) { n, _, e := syscall.Recvfrom(fd, buf, 0); if e != nil { time.Sleep(10*time.Millisecond); continue }; if n < 14+40+8 || binary.BigEndian.Uint16(buf[12:14]) != 0x86dd || buf[20] != 17 { continue }; udp := buf[14+40:n]; if binary.BigEndian.Uint16(udp[0:2]) != 53 || binary.BigEndian.Uint16(udp[2:4]) != 40002 { continue }; if bytes.Contains(udp[8:], []byte{203,0,113,53}) { fmt.Println("VPP DNS IPv6 packet response received"); return } }; panic("timed out waiting for VPP IPv6 DNS response")
}
func htons(v uint16) uint16 { return v<<8 | v>>8 }
EOF
...
docker exec "$name" sh -c '
  ...
  LY_ROUTE_DNS_SOURCE_ROUTES=/tmp/dns-source-routes.conf VCL_CONFIG=/tmp/vcl.conf VCL_VPP_API_SOCKET=/run/vpp/api.sock LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libvcl_ldpreload.so.25.10 /tmp/dns-vpp-proxy >/tmp/dns-vpp-proxy.log 2>&1 &
  ...
  kill "$(cat /tmp/dns-vpp-proxy.pid)" "$(cat /tmp/smartdns.pid)" 2>/dev/null || true
'

printf '%s\n' 'VPP SmartDNS IPv4/IPv6 UDP/TCP packet-flow verification passed'
