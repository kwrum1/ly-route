...
data_dir=${LY_ROUTE_DATA_DIR:-/var/lib/ly-route/orchestrator}
...
if [ ! -s "$tls_dir/admin.crt" ] || [ ! -s "$tls_dir/admin.key" ]; then
  openssl req -x509 -nodes -newkey rsa:2048 -days 30 \
    -subj "/CN=${LY_ROUTE_TLS_COMMON_NAME:-ly-route-orchestrator-demo}" \
...
export LY_ROUTE_ORCHESTRATOR_TRANSACTION_JOURNAL=${LY_ROUTE_ORCHESTRATOR_TRANSACTION_JOURNAL:-$data_dir/transparent-transaction.json}
export LY_ROUTE_ADMIN_USERNAME=${LY_ROUTE_ADMIN_USERNAME:-admin}
export LY_ROUTE_ADMIN_PASSWORD=${LY_ROUTE_ADMIN_PASSWORD:-LyRouteDemo2026!}
export LY_ROUTE_FORCE_PASSWORD_CHANGE=${LY_ROUTE_FORCE_PASSWORD_CHANGE:-false}
...
export LY_ROUTE_VPPCTL=${LY_ROUTE_VPPCTL:-/usr/local/bin/ly-route-vppctl-demo}
...
/usr/local/bin/ly-route-orchestrator &
control_pid=$!
...
until curl -fsS "http://127.0.0.1:${LY_ROUTE_API_PORT}/api/v1/health" >/dev/null; do
  ...
  if [ "$attempt" -ge 100 ]; then
    echo "ly-route-orchestrator did not become ready" >&2
    exit 1
  fi
  sleep 0.1
done
