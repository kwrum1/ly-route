...
async function saveModal(page) {
  ...
  try {
    await page.locator("#modalBackdrop").waitFor({ state: "hidden", timeout: 10000 });
  } catch (error) {
...
async function main() {
  ...
  page.on("request", (request) => {
    if (request.method() !== "GET" && request.url().includes("/api/v1/")) requests.push({ method: request.method(), url: request.url(), body: request.postData() || "" });
  });
  page.on("response", async (response) => {
    if (response.request().method() === "GET" || !response.url().includes("/api/v1/")) return;
    const body = await response.text().catch(() => "");
    console.log(`mutation ${response.status()} ${response.request().method()} ${response.url()} request=${response.request().postData() || ""} response=${body}`);
  });
  try {
    await login(page);
    await configureRole(page, "eth1", "lan");
    await configureRole(page, "eth2", "wan");
    ...
    await selectInterface(page, "[data-lan-interface]", "eth1");
    ...
    await selectInterface(page, "[data-wan-interface]", "eth2");
    ...
    assert.equal(pppoe.interface_id, "eth2");
    ...
    for (let attempt = 0; attempt < 30; attempt += 1) {
      const interfaces = dockerExec(gatewayContainer, "sh", "-lc", "ip -o link show | awk -F': ' '$2 ~ /^ppp-/ {print $2}'");
      if (interfaces) {
        pppInterface = interfaces.split("\\n")[0].trim();
        break;
      }
      await new Promise((resolve) => setTimeout(resolve, 1000));
    }
    assert.ok(pppInterface, "UI-applied PPPoE WAN must create a PPP interface in the gateway container");
    ...
    await writeFile(join(evidenceDir, "summary.json"), `${JSON.stringify({ result: "passed", pppInterface, serverInterfaces, requestCount: requests.length }, null, 2)}\n`);
