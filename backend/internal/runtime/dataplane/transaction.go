...
func (transaction *Transaction) Apply(ctx context.Context, request Request) (Receipt, error) {
	...
	if request.Path.Tier == vpp.DataplaneTierNative {
		...
		if transaction.current != nil {
			return Receipt{}, fmt.Errorf("%w: active DPDK path must be reconciled before selecting native", ErrTransactionFailed)
		}
...
func (transaction *Transaction) Rollback(ctx context.Context, transactionID string) (Receipt, error) {
	...
	return Receipt{TransactionID: transactionID, Tier: vpp.DataplaneTierDPDK, Status: "rolled_back", AppliedAt: transaction.now(), Attachments: len(snapshot.Devices), RolledBack: true, Changed: true}, nil
}
