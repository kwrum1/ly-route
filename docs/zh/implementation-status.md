...
更新日期：2026-08-19
当前代码基线：以仓库 `main` 当前提交和带指纹的热修复产物为准。

> 本文下方的 2026-08-01/02 数字和清单是历史快照，不是当前验收结论。当前 ESXi 批次证据位于 `.sisyphus/full-acceptance/evidence/gateway-live-batch-current/`；本轮只完成 API/运行态读取，独立客户端探针未提供的项目统一记为 `FIXTURE_FAIL`，不得合并进通过率。
...
历史快照曾记录统一容器/浏览器子集和发布矩阵通过率；这些数字不再代表当前版本。当前版本必须以本轮批次证据和独立客户端包流结果重新计算，未完成的夹具不得记为产品失败，也不得记为通过。
...
## 3. 出口网关盘点（历史实现盘点，非当前验收结论）

> 本节的“已完成”是历史代码/容器证据摘要。当前现场批次已重新执行；任何没有对应最新独立客户端证据的项目，都保持“未验收”，尤其是 DHCP、DNS 分流、代理、遥测和故障恢复。
...
| 代理出口 | 服务与容器闭环 | 单节点/订阅、固定节点、订阅内自动选择、订阅内 RTT Top N、按 `1/RTT` 归一化权重、五元组稳定哈希分配、失败节点剔除与重编排、underlay 约束；明确不支持多代理 WAN 群组 | 代理 DNS 选线、长稳、防抖参数和硬件性能 |
