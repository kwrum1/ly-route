...
rootfs:
	./scripts/build-rootfs.sh --product "$(PRODUCT)" --arch "$(ARCH)" --suite "$(SUITE)" --out "$(OUT)"

rootfs-amd64:
	./scripts/build-rootfs.sh --product "$(PRODUCT)" --arch amd64 --suite "$(SUITE)" --out "$(OUT)"

rootfs-arm64:
	./scripts/build-rootfs.sh --product "$(PRODUCT)" --arch arm64 --suite "$(SUITE)" --out "$(OUT)"

disk-image:
	./scripts/build-disk-image.sh --product "$(PRODUCT)" --rootfs "$(ROOTFS)" --suite "$(SUITE)" --out "$(OUT)"
